...
volatile uint8_t rpmsg_ept_ready = 0;
/* USER CODE END PV */
...
/* USER CODE BEGIN PFP */
extern void logger_rpmsg_bind_cb(struct rpmsg_device *rdev,
...
int rpmsg_recv_callback(struct rpmsg_endpoint *ept, void *data,
                         size_t len, uint32_t src, void *priv)
{
  /* Data logger chủ yếu gửi 1 chiều M4->A7, không cần xử lý gì ở đây */
